...
def load_documents_from_json() -> Tuple[List[Document], List[Document]]:
    ...
    if not os.path.exists(documents_path):
        raise FileNotFoundError(f"Файл {documents_path} не найден. Сначала запустите stage 1.")
...
def split_documents_into_chunks(
    data_md: List[Document],
    data_pdf: List[Document]
) -> List[Document]:
    """
...
def main():
    ...
    train_chunks, val_chunks = split_chunks_train_test(all_chunks, train_size=TRAIN_TEST_SPLIT)
